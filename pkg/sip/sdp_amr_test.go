...
func TestGenerateAnswer_AMRRejectsOctetAlignOnly(t *testing.T) {
	...
	c1, _ := newUDPPipe()
	port := newTestPort(t, logger.NewTestLogger(t), c1, &MediaOptions{
		IP:     newIP("1.1.1.1"),
...
a=fmtp:96 octet-align=1;mode-change-capability=2
`)
	_, err := port.GenerateAnswer(offer)
	require.Error(t, err)
}

func TestGenerateAnswer_AMRAcceptsBandwidthEfficientAndEchoesFmtp(t *testing.T) {
	codecs := amrEnabledCodecs()
...
a=fmtp:97 0-15
`)
	answerData, err := port.GenerateAnswer(offer)
	require.NoError(t, err)
	s := string(answerData)
