...
func reinviteOfferDropsNegotiatedAudio(mp MediaPort, codecs *msdk.CodecSet, log logger.Logger, body []byte) bool {
	...
	if negotiated == nil || negotiated.Codec == nil {
		return false
	}
	...
	for _, c := range offer.MediaDesc.Codecs {
		if c.Type != negotiated.Type {
			continue
		}
		if c.Codec == nil || c.Codec.Info().SDPName == negotiated.Codec.Info().SDPName {
			return false
		}
	}
	log.Infow("rejecting re-INVITE that removes negotiated audio codec",
		"negotiatedType", negotiated.Type,
		"negotiatedCodec", negotiated.Codec.Info().SDPName,
	)
	return true
}
