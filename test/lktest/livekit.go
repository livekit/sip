...
func (lk *LiveKit) ConnectParticipant(t TB, room, identity string, cb *RoomParticipantCallback) *Participant {
	...
	var err error
	p.mixIn, err = mixer.NewMixer(pw, rtp.DefFrameDur, nil, 1, mixer.WithOutputChannel())
	if err != nil {
		t.Fatal(err)
	}
	...
	p.mixOut, err = mixer.NewMixer(track, rtp.DefFrameDur, nil, 1, mixer.WithOutputChannel())
	if err != nil {
		t.Fatal(err)
	}
